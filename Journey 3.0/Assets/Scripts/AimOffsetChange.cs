...
public class AimOffsetChange : MonoBehaviour
{
    ...
    [NotNull] private DetectPlayer _trigger1;
    ...
    {
        valueIncreasing =
            false; //used to ensure offset value is not increasing in IncreaseValue() while it is decreasing here
        if (startOffset > newOffset) //e.g SO = 0, NO = -28
        {
            while (currentOffset > newOffset + 0.05f && valueIncreasing == false
            ) //-0.05 is used because offset will never reach newoffset value
            {
        ...
        {
            while (currentOffset > startOffset + 0.05f && valueIncreasing == false
            ) //+0.05 is used because offset will never reach the startoffset value
            {
    ...
    IEnumerator IncreaseValue()
    {
        ...
        if (startOffset > newOffset)
        {
            ...
            while (currentOffset < startOffset - 0.05f && valueIncreasing
            ) //+0.05 is used because offset will never reach the startoffset value
            {
                targetOffset = startOffset;
                AssignAxis(); //lerp offset of the lookat target to the startoffset value over time
        ...
        {
            while (currentOffset < newOffset - 0.05f && valueIncreasing
            ) //-0.05 is used because offset will never reach newoffset value
            {
                targetOffset = newOffset;
                AssignAxis(); //lerp offset of the lookat target to the newoffset value over time
    ...
    IEnumerator
        ChangeBool(int framesToWait) //not used - Used to change a bool - waits for an amount of time before this can be called again
    {
